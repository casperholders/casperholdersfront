<template>
  <v-app>
    ...
    >
      ...
      <div v-bind:key="name" v-for="(items, name) in links">
        <v-divider></v-divider>
          ...
          <v-list-item>
            <v-tooltip bottom>
              <template v-slot:activator="{ on, attrs }">
                <v-list-item-avatar :color="connected ? 'green' : 'red'"
                                    v-bind="attrs"
                                    v-on="on">
...
export default {
  ...
  mounted() {
    this.signerStatus();
    this.timer = setInterval(this.signerStatus, 60000);
  ...
  methods: {
    ...
    signerStatus: function () {
      console.log("Check signer status")
      Signer.isConnected().then((res) => {
        if (res) {
          this.connected = true
          Signer.getVersion().then((res) => {
            this.version = res
          })
        } else {
