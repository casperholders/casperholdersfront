describe('Transfer', () => {
  ...
  it('Should not let you do a transfer', () => {
    ...
    cy.get('#address').type('NotValidAddress').parents('.v-input__control').find('.v-messages__message').should('contain', 'Error: Unsupported type of public key');
    ...
    cy.get('#amount').type('{selectall}{del}1').parents('.v-input__control').find('.v-messages__message').should('contain', 'Amount must be at least');
    cy.get('#transferID').type('NotValidTransferID').parents('.v-input__control').find('.v-messages__message').should('contain', 'Transfer ID must be a number');
    cy.get('#transferID').type('{selectall}{del}').parents('.v-input__control').find('.v-messages__message').should('contain', 'Transfer ID is required');
    cy.get('#min').click();
